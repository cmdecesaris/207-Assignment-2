...
```{r}
anova.1 = aov(wage~factor(occupation)+factor(ethnicity)+factor(occupation):factor(ethnicity), data = Wage )
anova.1$coeff
summary(anova.1)
```
...
- (v) $ cov( (\alpha\beta)_{ij},  (\alpha\beta)_{ij'}) = \sigma^2_{ \alpha_i}$,
...
4. ${\rm cov}(Y_{ijk},Y_{ij'\alpha\beta}/a$, (vi) ${\rm cov}( (\alpha\beta)_{ij},  (\alpha\beta)_{i'j'})=0$,  if $i\neq i'$ and $j\neq j'$, (vii)$\{\epsilon_{ijk}\}$ are i.i.d. $N(0,\sigma^2)$, and (viii)  k'})=0$ for $j\neq j'$.
